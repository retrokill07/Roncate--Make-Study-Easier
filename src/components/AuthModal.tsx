...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Switch to "Complete Profile" mode instead of hardcoding defaults
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isCompletingGoogleProfile && pendingGoogleUser) {
        // Complete Google Profile Logic
        ...
        await setDoc(doc(db, "users", pendingGoogleUser.uid), profileData);
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-indigo-600 p-6 text-center">
          ...
          <h2 className="text-2xl font-bold text-white">
            {isCompletingGoogleProfile ? 'Complete Your Profile' : (isSignup ? 'Create Account' : 'Welcome Back')}
          </h2>
          <p className="text-indigo-100 text-sm">
            {isCompletingGoogleProfile ? 'Just a few more details to get started' : (isSignup ? 'Start your adaptive learning journey' : 'Login to access your dashboard')}
          </p>
        </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          ...
          {resetSent ? (
            ...
            {(isSignup || isCompletingGoogleProfile) && (
              <div>
            ...
            >
              {loading ? 'Processing...' : (isCompletingGoogleProfile ? 'Finish Setup' : (isSignup ? 'Create Account' : 'Login'))}
            </button>
          </form>
